		...
		async () => {
			...
			try {
				...
				for (const eventDoc of eventDocs) {
					...
					const expensesSnapshot = await eventDoc.ref
						.collection("expenses")
						.get();
						...
						const paymentData = expenseData.oneOffPayment
							? {
									...
									expense_payment_description:
										expenseData.oneOffPayment.description,
									...
									expense_payment_date:
										expenseData.oneOffPayment.date?.toDate(),
				...
				if (expensesData.length === 0) {
					throw new HttpsError(
						"not-found",
						`No expense data found for ${userEmail}.`,
					);
				}
				...
				const tempFilePath = path.join(
					os.tmpdir(),
					`expenses-export-${userId}-${timestamp}.csv`,
				);
				fs.writeFileSync(tempFilePath, csv);

				// Upload to Firebase Storage
				const defaultBucket =
					params?.storageBucket?.value() || storage.bucket().name;
