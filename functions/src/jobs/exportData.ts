      ...
      for (const eventDoc of eventDocs) {
        ...
        for (const expenseDoc of expenseDocs) {
          ...
          const paymentData = expenseData.oneOffPayment ? {
            ...
            expense_payment_name: expenseData.paymentSchedule[expenseData.paymentSchedule.length - 1].name,
            expense_payment_description: expenseData.paymentSchedule[expenseData.paymentSchedule.length - 1].description,
            expense_payment_amount: expenseData.paymentSchedule.reduce((total: number, payment: any) => total + payment.amount, 0) ,
            expense_payment_isPaid: expenseData.paymentSchedule[expenseData.paymentSchedule.length - 1].isPaid,
            expense_payment_date: expenseData.paymentSchedule[expenseData.paymentSchedule.length - 1].date?.toDate(),
            expense_payment_method: expenseData.paymentSchedule[expenseData.paymentSchedule.length - 1].method,
          } : {};
